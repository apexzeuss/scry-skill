...
to keep context small) to two focused skill files:
...
Both call the runnable logic in `scripts/` (`scan_wallet.ts`, `watch_deployments.ts`),
with scoring weights documented in `rules/risk-thresholds.md`. Run `bash install.sh`
once to install dependencies. A faster RPC is optional: set `SOLANA_RPC_URL`.
